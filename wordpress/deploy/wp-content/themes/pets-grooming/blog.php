...
if ( function_exists( 'pets_grooming_elementor_is_preview' ) && pets_grooming_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			pets_grooming_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	pets_grooming_new_main_query(
		array(
			'post_type'      => pets_grooming_get_theme_option( 'post_type' ),
			'category'       => pets_grooming_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'pets_grooming_filter_get_template_part', pets_grooming_blog_archive_get_template() ) );
}
