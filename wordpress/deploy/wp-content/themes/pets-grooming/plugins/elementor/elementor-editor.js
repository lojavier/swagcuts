...
jQuery( document ).ready(
	function() {
		...
		pets_grooming_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="pets_grooming_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.pets_grooming_post_editor' );
						if ( link.length > 0 ) {
							pets_grooming_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.pets_grooming_post_editor', function(e) {
						pets_grooming_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('pets_grooming_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function pets_grooming_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
