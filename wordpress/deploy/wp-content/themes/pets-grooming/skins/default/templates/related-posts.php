...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $pets_grooming_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	<div class="post_header entry-header">
		<h6 class="post_title entry-title"><a href="<?php echo esc_url( $pets_grooming_link ); ?>"><?php
			if ( '' == get_the_title() ) {
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<span class="post_date"><a href="<?php echo esc_url( $pets_grooming_link ); ?>"><?php echo wp_kses_data( pets_grooming_get_date() ); ?></a></span>
			<?php
		}
		?>
	</div>
