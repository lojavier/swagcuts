...
if ( ! function_exists( 'pets_grooming_storage_get' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_get( $var_name, $default = '' ) {
		global $PETS_GROOMING_STORAGE;
		return isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ? $PETS_GROOMING_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'pets_grooming_storage_set' ) ) {
	/**
...
if ( ! function_exists( 'pets_grooming_storage_empty' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $PETS_GROOMING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $PETS_GROOMING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_isset' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $PETS_GROOMING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $PETS_GROOMING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_unset' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $PETS_GROOMING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
				unset( $PETS_GROOMING_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_inc' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_inc( $var_name, $value = 1 ) {
		global $PETS_GROOMING_STORAGE;
		if ( empty( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = 0;
		}
		$PETS_GROOMING_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'pets_grooming_storage_concat' ) ) {
	/**
...
if ( ! function_exists( 'pets_grooming_storage_get_array' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $PETS_GROOMING_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ? $PETS_GROOMING_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_set_array' ) ) {
	/**
	...
	function pets_grooming_storage_set_array( $var_name, $key, $value ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$PETS_GROOMING_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'pets_grooming_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function pets_grooming_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$PETS_GROOMING_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$PETS_GROOMING_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_merge_array( $var_name, $key, $value ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array_merge( $PETS_GROOMING_STORAGE[ $var_name ], $value );
		} else {
			$PETS_GROOMING_STORAGE[ $var_name ][ $key ] = array_merge( $PETS_GROOMING_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			pets_grooming_array_insert_after( $PETS_GROOMING_STORAGE[ $var_name ], $after, $key );
		} else {
			pets_grooming_array_insert_after( $PETS_GROOMING_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function pets_grooming_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			pets_grooming_array_insert_before( $PETS_GROOMING_STORAGE[ $var_name ], $before, $key );
		} else {
			pets_grooming_array_insert_before( $PETS_GROOMING_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_push_array' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_push_array( $var_name, $key, $value ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $PETS_GROOMING_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ) {
				$PETS_GROOMING_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $PETS_GROOMING_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $PETS_GROOMING_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $PETS_GROOMING_STORAGE[ $var_name ] ) && is_array( $PETS_GROOMING_STORAGE[ $var_name ] ) && count( $PETS_GROOMING_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $PETS_GROOMING_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) && is_array( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) && count( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'pets_grooming_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function pets_grooming_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $PETS_GROOMING_STORAGE;
		if ( ! isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ][ $key ] = 0;
		}
		$PETS_GROOMING_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'pets_grooming_storage_concat_array' ) ) {
	/**
	...
	function pets_grooming_storage_concat_array( $var_name, $key, $value ) {
		...
		if ( empty( $PETS_GROOMING_STORAGE[ $var_name ][ $key ] ) ) {
			$PETS_GROOMING_STORAGE[ $var_name ][ $key ] = '';
		}
		$PETS_GROOMING_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'pets_grooming_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $PETS_GROOMING_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ? $PETS_GROOMING_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PETS_GROOMING_STORAGE[ $var_name ] ) ? $PETS_GROOMING_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'pets_grooming_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function pets_grooming_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $PETS_GROOMING_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $PETS_GROOMING_STORAGE[ $var_name ]->$prop ) ? $PETS_GROOMING_STORAGE[ $var_name ]->$prop : $default;
	}
}
