...
// Current version
if ( ! defined( 'TRX_ADDONS_VERSION' ) ) define( 'TRX_ADDONS_VERSION', '2.38.3' );
	...
	function trx_addons_grab_page_end( $html = '' ) {
		...
		if ( ! empty( $TRX_ADDONS_STORAGE['capture_head_html'] ) && ! empty( $TRX_ADDONS_STORAGE['capture_body_html'] ) ) {
			...
			$TRX_ADDONS_STORAGE['capture_head_html'] = preg_replace( '/(\r?\n){2,}/', '$1', $TRX_ADDONS_STORAGE['capture_head_html'] )
				...
				// Add all captured links from other 3rd-party plugins before theme's links
...
if ( ! function_exists( 'trx_addons_move_own_styles_to_head' ) ) {
	...
	function trx_addons_move_own_styles_to_head() {
		...
		foreach( array( 'head', 'body' ) as $part ) {
			$TRX_ADDONS_STORAGE["capture_{$part}_html"] = preg_replace_callback(
				'/'
				. '(<link [^>]*href=[\'"][^\'"]*trx_addons[^\'"]*[\'"][^>]*>)'
				. '|(<link [^>]*id=[\'"]trx_addons-[^\'"]*[\'"][^>]*>)'
				. '|(<style[^>]*id=[\'"]trx_addons-[^\'"]*[\'"][^>]*>[\s\S]*<\/style>)'
				. '/Uix',
				function( $matches ) use ( $theme_slug, $child_theme_slug ) {
					if ( ! empty( $matches[0] ) && strpos( $matches[0], '/' . $theme_slug . '/' ) === false && strpos( $matches[0], '/' . $child_theme_slug . '/' ) === false ) {
						global $TRX_ADDONS_STORAGE;
						$TRX_ADDONS_STORAGE[ strpos( $matches[0], '<link ' ) === false || ( strpos( $matches[0], 'responsive-css' ) === false && strpos( $matches[0], 'responsive.css' ) === false )
							? 'capture_own_links'
				...
				$TRX_ADDONS_STORAGE["capture_{$part}_html"]
			);
		}
	}
}
