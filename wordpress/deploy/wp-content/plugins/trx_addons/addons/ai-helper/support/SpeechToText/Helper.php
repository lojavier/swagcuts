...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		 ...
		 */
		public static function get_transcription_model() {
		...
		public static function is_allowed() {
			$allowed = false;
			$model = self::get_transcription_model();
			if ( ! is_admin() && ! empty( $model ) ) {
		 ...
		 */
		public function check_sc( $sc, $prefix = '' ) {
			$found = false;
			if ( ! empty( $sc ) ) {
		...
		public function enqueue_scripts( $force = false, $sc = '' ) {
			if ( $this->check_sc( $sc ) && self::is_allowed() ) {
				...
				wp_enqueue_script( 'trx_addons-ai-helper-speech-to-text', trx_addons_get_file_url( TRX_ADDONS_PLUGIN_ADDONS . 'ai-helper/support/SpeechToText/assets/js/index.js' ), array( 'jquery', 'trx_addons-utils' ), null, true );
		...
		public function localize_script( $vars ) {
			if ( self::is_allowed() ) {
				...
				$vars['msg_stt_button_title'] = esc_attr__( 'Press and dictate the desired text into the microphone.', 'trx_addons' );
			}
			return $vars;
		}
		...
		public function fetch_answer() {
			...
			$model = trx_addons_get_value_gp( 'fetch_model', self::get_transcription_model() );
			...
			if ( count( $audio_models ) > 0 ) {
				$element->add_control(
					...
					[
						...
						'label_block' => false,
						'description' => __( 'Allow visitors to dictate prompts using a microphone', 'trx_addons' ),
		...
		public function stt_button_layout( $args, $prompt_field_id ) {
			if ( ! empty( $args['allow_voice_input'] ) ) {
				...
				<a href="#" class="trx_addons_ai_helper_stt_button <?php echo esc_attr( $voice_input_icon ); ?>"
					title="<?php esc_attr_e( 'Press and dictate the desired text into the microphone.', 'trx_addons' ); ?>"
