...
if ( ! function_exists( 'pets_grooming_array_get_first' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'pets_grooming_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'pets_grooming_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'pets_grooming_array_from_list' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'pets_grooming_array_slice' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'pets_grooming_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * pets_grooming_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * pets_grooming_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function pets_grooming_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'pets_grooming_array_insert_after' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = pets_grooming_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: pets_grooming_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'pets_grooming_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function pets_grooming_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'pets_grooming_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'pets_grooming_array_str_replace' ) ) {
	/**
	 ...
	 */
	function pets_grooming_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = pets_grooming_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'pets_grooming_generate_id' ) ) {
	/**
	 ...
	 */
	function pets_grooming_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'pets_grooming_get_tag' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'pets_grooming_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'pets_grooming_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( pets_grooming_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'pets_grooming_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function pets_grooming_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'pets_grooming_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function pets_grooming_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = pets_grooming_fga( $css );
...
if ( ! function_exists( 'pets_grooming_get_column_class' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'pets_grooming_get_svg_from_file' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_svg_from_file( $svg ) {
		if ( pets_grooming_is_url( $svg ) && ! pets_grooming_is_external_url( $svg ) ) {
			$svg_path = pets_grooming_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
		$content = pets_grooming_fgc( $svg );
		preg_match( "#<\s*?svg\b[^>]*>(.*?)</svg\b[^>]*>#is", $content, $matches );
		return ! empty( $matches[0]) ? str_replace( array( "\r", "\n" ), array( '', ' ' ), $matches[0] ) : '';
	}
}

if ( ! function_exists( 'pets_grooming_external_links_target' ) ) {
	/**
	 ...
	 */
	function pets_grooming_external_links_target( $admin = false ) {
...
if ( ! function_exists( 'pets_grooming_stripslashes' ) ) {
	/**
	 ...
	 */
	function pets_grooming_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
		...
		if ( is_array( $val ) ) {
			foreach ( $val as $k => $v ) {
				$val[ $k ] = pets_grooming_stripslashes( $v );
			}
		} else {
...
if ( ! function_exists( 'pets_grooming_get_value_gp' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'pets_grooming_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'pets_grooming_get_value_gps' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'pets_grooming_get_session_value' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'pets_grooming_set_session_value' ) ) {
	/**
	 ...
	 */
	function pets_grooming_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'pets_grooming_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function pets_grooming_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'pets_grooming_get_cookie' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_cookie( $name, $defa = '' ) {
		return pets_grooming_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'pets_grooming_set_cookie' ) ) {
	/**
	 ...
	 */
	function pets_grooming_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'pets_grooming_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//pets_grooming_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//pets_grooming_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'pets-grooming' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'pets_grooming_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'pets_grooming_hex2rgba' ) ) {
	/**
	 ...
	 */
	function pets_grooming_hex2rgba( $hex, $alpha ) {
		$rgb = pets_grooming_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'pets_grooming_rgba2hex' ) ) {
	/**
	 ...
	 */
	function pets_grooming_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'pets_grooming_hex2hsb' ) ) {
	/**
	 ...
	 */
	function pets_grooming_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = pets_grooming_rgb2hsb( pets_grooming_hex2rgb( $hex ) );
...
if ( ! function_exists( 'pets_grooming_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'pets_grooming_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'pets_grooming_rgb2hex' ) ) {
	/**
	 ...
	 */
	function pets_grooming_rgb2hex( $rgb ) {
		$hex = array(
			dechex( $rgb['r'] ),
			dechex( $rgb['g'] ),
			dechex( $rgb['b'] ),
...
if ( ! function_exists( 'pets_grooming_hsb2hex' ) ) {
	/**
	...
	function pets_grooming_hsb2hex( $hsb ) {
		return pets_grooming_rgb2hex( pets_grooming_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'pets_grooming_date_to_sql' ) ) {
	/**
	 ...
	 */
	function pets_grooming_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'pets_grooming_format_price' ) ) {
	/**
	 ...
	 */
	function pets_grooming_format_price( $price ) {
...
if ( ! function_exists( 'pets_grooming_num2size' ) ) {
	/**
	 ...
	 */
	function pets_grooming_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'pets_grooming_size2num' ) ) {
	/**
	 ...
	 */
	function pets_grooming_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'pets_grooming_parse_num' ) ) {
	/**
	 ...
	 */
	function pets_grooming_parse_num( $str ) {
...
if ( ! function_exists( 'pets_grooming_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function pets_grooming_prepare_macros( $str ) {
...
if ( ! function_exists( 'pets_grooming_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function pets_grooming_remove_macros( $str ) {
		return empty( $str ) ? '' : str_replace(
										array( '{{', '}}', '((', '))', '||' ),
										array( '', '', '', '', ' ' ),
										$str
...
if ( ! function_exists( 'pets_grooming_is_on' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_on( $prm ) {
		return is_array( $prm )
					? count( $prm ) > 0
...
if ( ! function_exists( 'pets_grooming_is_off' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_off( $prm ) {
		return is_array( $prm )
					? count( $prm ) == 0
					: empty( $prm )
...
if ( ! function_exists( 'pets_grooming_is_inherit' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'pets_grooming_strshort' ) ) {
	/**
	 ...
	 */
	function pets_grooming_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = pets_grooming_strip_tags( $str );
...
if ( ! function_exists( 'pets_grooming_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function pets_grooming_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'pets_grooming_strip_tags' ) ) {
	/**
	 ...
	 */
	function pets_grooming_strip_tags( $str ) {
...
if ( ! function_exists( 'pets_grooming_excerpt' ) ) {
	/**
	 ...
	 */
	function pets_grooming_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return pets_grooming_strwords( pets_grooming_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'pets_grooming_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function pets_grooming_unserialize_recover( $str ) {
...
if ( ! function_exists( 'pets_grooming_unserialize' ) ) {
	/**
	 ...
	 */
	function pets_grooming_unserialize( $str, $disable_objects = false ) {
		if ( ! empty( $str ) && is_serialized( $str ) ) {
			...
			if ( $disable_objects ) {
				...
				$data = @unserialize( $str, $args );
					...
					if ( false === $data ) {
					$data = @unserialize( pets_grooming_unserialize_recover( $str ), $args );
					}
					// Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
					if ( false === $data ) {
					$data = @unserialize( pets_grooming_unserialize_recover( $str2 ), $args );
					}
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'pets_grooming_str_replace' ) ) {
	/**
	 ...
	 */
	function pets_grooming_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = pets_grooming_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( pets_grooming_str_replace( $from, $to, pets_grooming_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'pets_grooming_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function pets_grooming_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'pets_grooming_tags_count' ) ) {
	/**
	 ...
	 */
	function pets_grooming_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'pets_grooming_getimagesize' ) ) {
	/**
	 ...
	 */
	function pets_grooming_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = pets_grooming_is_url( $url ) ? pets_grooming_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'pets_grooming_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function pets_grooming_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = pets_grooming_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'pets_grooming_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function pets_grooming_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		if ( isset( $_wp_additional_image_sizes ) && count( $_wp_additional_image_sizes ) && in_array( $thumb_size, array_keys( $_wp_additional_image_sizes ) ) ) {
			if ( empty( $_wp_additional_image_sizes[ $thumb_size ]['height'] ) || empty( $_wp_additional_image_sizes[ $thumb_size ]['crop'] ) ) {
				$image_id = pets_grooming_attachment_url_to_postid( $url );
				if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
					$attach = wp_get_attachment_image_src( $image_id, $thumb_size );
					if ( ! empty( $attach[0] ) ) {
						$pi = pathinfo( $attach[0] );
						$pi['dirname'] = pets_grooming_remove_protocol_from_url( $pi['dirname'] );
						$parts = explode( '-', $pi['filename'] );
					}
				}
			} else {
		...
		$new_url = pets_grooming_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = pets_grooming_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = pets_grooming_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = pets_grooming_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'pets_grooming_get_thumb_size' ) ) {
	/**
	 * Return a theme-specific image thumb size name with a prefix and a multiplier (if a visitor have a retina device).
	 *
	 * For example:
	 *
	 * 'masonry' -> 'pets-grooming-thumb-masonry' or 'pets-grooming-thumb-masonry-@retina'
	 *
	 ...
	 */
	function pets_grooming_get_thumb_size( $ts ) {
		$thumb_sizes = pets_grooming_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "pets-grooming-thumb-{$ts}" ] ) ) {
			$ts = 'pets-grooming-thumb-' . $ts;
		}
		$retina = pets_grooming_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'pets_grooming_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'pets_grooming_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'pets-grooming-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'pets_grooming_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'pets-grooming-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'pets_grooming_get_closest_thumb_size' ) ) {
	 ...
	 */
	function pets_grooming_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'pets_grooming_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function pets_grooming_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = pets_grooming_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'pets_grooming_get_post_image' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'pets_grooming_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function pets_grooming_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = pets_grooming_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = pets_grooming_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = pets_grooming_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( pets_grooming_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = pets_grooming_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'pets_grooming_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function pets_grooming_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'pets_grooming_get_post_audio_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function pets_grooming_get_post_audio_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_audio_list_first' ) ? trx_addons_get_post_audio_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'pets_grooming_get_post_video' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function pets_grooming_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = pets_grooming_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = pets_grooming_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = pets_grooming_get_tag( $post_text, '<video', '</video>' );
				if ( empty( $img ) ) {
					$sc = pets_grooming_get_tag( $post_text, '[video', '[/video]' );
					if ( empty( $sc ) ) {
						$sc = pets_grooming_get_tag( $post_text, '[trx_widget_video', '' );
					}
					if ( ! empty( $sc ) ) {
						$img = do_shortcode( $sc );
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp\\:trx-addons\\/video.+"link"\\:"([^"]*)"/is', $post_text, $matches ) ) {
						$img = pets_grooming_get_embed_video( $matches[1][0] );
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp:core-embed\\/(youtube|vimeo|dailymotion|facebook).+"url":"([^"]*)"/is', $post_text, $matches ) ) {
						$img = pets_grooming_get_embed_video( $matches[2][0] );	// , true
					}
					if ( empty( $img ) && preg_match_all( '/<!-- wp:embed {"url":"([^"]*(youtube|vimeo|dailymotion|facebook)[^"]*)"/is', $post_text, $matches ) ) {
						$img = pets_grooming_get_embed_video( $matches[1][0] );	// , true
					}
		...
		return apply_filters( 'pets_grooming_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'pets_grooming_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function pets_grooming_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'pets_grooming_get_post_video_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function pets_grooming_get_post_video_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_video_list_first' ) ? trx_addons_get_post_video_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'pets_grooming_get_post_iframe' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function pets_grooming_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = pets_grooming_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = pets_grooming_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'pets_grooming_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'pets_grooming_get_embed_video' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
		} else if ( pets_grooming_is_from_uploads( $video ) ) {
			$embed_video  = '<video'
								. ' src="' . esc_url( $video ) . '"'
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = pets_grooming_add_to_url( $video, pets_grooming_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'pets_grooming_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'pets_grooming_filter_video_width', 325, 'tiktok' ) : pets_grooming_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'pets_grooming_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'pets_grooming_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function pets_grooming_get_embed_video_tag_name() {
...
if ( ! function_exists( 'pets_grooming_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function pets_grooming_make_video_autoplay( $video, $muted = false ) {
			...
			if ( false !== $pos ) {
				if ( preg_match( '/(<' . esc_html( $tag ) . '[^>]*src=[\'"])([^\'"]+)([\'"][^>]*>)(.*)/is', $video, $matches ) ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = pets_grooming_add_to_url(
							$matches[2],
							array_merge(
								array(
								...
								$muted
									? array_merge(
										pets_grooming_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'pets_grooming_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_from_uploads( $url ) {
		$url          = pets_grooming_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = pets_grooming_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'pets_grooming_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'pets_grooming_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'pets_grooming_fpc' ) ) {
	/**
	 ...
	 */
	function pets_grooming_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				return trx_addons_fpc( $file, $data, $flag );
			} else {
				$file = pets_grooming_prepare_path( $file );
...
if ( ! function_exists( 'pets_grooming_fgc' ) ) {
	/**
	 ...
	 */
	function pets_grooming_fgc( $file ) {
		if ( ! empty( $file ) ) {
...
if ( ! function_exists( 'pets_grooming_fga' ) ) {
	/**
	 ...
	 */
	function pets_grooming_fga( $file ) {
		if ( ! empty( $file ) ) {
...
if ( ! function_exists( 'pets_grooming_mkdir' ) ) {
	/**
	 ...
	 */
	function pets_grooming_mkdir( $path ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_mkdir' ) ) {
				return trx_addons_mkdir( $path );
			} else {
				$path = pets_grooming_prepare_path( $path );
				if ( ! is_dir( $path ) ) {
...
if ( ! function_exists( 'pets_grooming_unlink' ) ) {
	/**
	 ...
	 */
	function pets_grooming_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( is_dir( $path ) ) {
					...
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							pets_grooming_unlink( "$path/$file" );
						}
					}
					rmdir( $path );
					return true;
...
if ( ! function_exists( 'pets_grooming_copy' ) ) {
	/**
	 ...
	 */
	function pets_grooming_copy( $src, $dst ) {
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				return trx_addons_copy( $src, $dst );
			} else {
				pets_grooming_unlink( $dst );
				if ( is_dir( $src ) ) {
					if ( ! is_dir( $dst ) ) {
						pets_grooming_mkdir( $dst );
					}
					$files = scandir( $src, SCANDIR_SORT_NONE );
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							pets_grooming_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'pets_grooming_unzip_file' ) ) {
	/**
	 ...
	 * @return bool         Return true if no error occurs.
	 */
	function pets_grooming_unzip_file( $zip, $dest ) {
...
if ( ! function_exists( 'pets_grooming_retrieve_json' ) ) {
	/**
	 ...
	 */
	function pets_grooming_retrieve_json( $url ) {
		return function_exists( 'trx_addons_retrieve_json' ) ? trx_addons_retrieve_json( $url ) : '';
	}
}

if ( ! function_exists( 'pets_grooming_esc' ) ) {
	/**
	 ...
	 */
	function pets_grooming_esc( $name, $local = false ) {
		return str_replace(
...
if ( ! function_exists( 'pets_grooming_sanitize_slug' ) ) {
	/**
	 ...
	 */
	function pets_grooming_sanitize_slug( $slug ) {
		return str_replace( array( ':', '.', '/', '\\' ), '', pets_grooming_esc( $slug ) );
	}
}

if ( ! function_exists( 'pets_grooming_prepare_path' ) ) {
	/**
	 ...
	 */
	function pets_grooming_prepare_path( $path ) {
		return str_replace( array( '\\', '/' ), defined( 'DIRECTORY_SEPARATOR' ) ? DIRECTORY_SEPARATOR : '/', $path );
	}
}

if ( ! function_exists( 'pets_grooming_check_min_file' ) ) {
	/**
	 ...
	 */
	function pets_grooming_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && pets_grooming_is_off( pets_grooming_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && pets_grooming_is_off( pets_grooming_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'pets_grooming_get_file_dir' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! PETS_GROOMING_ALLOW_SKINS && ! pets_grooming_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'pets_grooming_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( PETS_GROOMING_CHILD_DIR != PETS_GROOMING_THEME_DIR && file_exists( PETS_GROOMING_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? PETS_GROOMING_CHILD_URL : PETS_GROOMING_CHILD_DIR ) . pets_grooming_check_min_file( $file, PETS_GROOMING_CHILD_DIR );
			} elseif ( file_exists( PETS_GROOMING_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? PETS_GROOMING_THEME_URL : PETS_GROOMING_THEME_DIR ) . pets_grooming_check_min_file( $file, PETS_GROOMING_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'pets_grooming_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function pets_grooming_get_file_url( $file ) {
		return pets_grooming_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'pets_grooming_get_file_ext' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'pets_grooming_get_file_name' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'pets_grooming_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'pets_grooming_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( PETS_GROOMING_CHILD_DIR != PETS_GROOMING_THEME_DIR && is_dir( PETS_GROOMING_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? PETS_GROOMING_CHILD_URL : PETS_GROOMING_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( PETS_GROOMING_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? PETS_GROOMING_THEME_URL : PETS_GROOMING_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'pets_grooming_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'pets_grooming_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function pets_grooming_get_folder_url( $folder ) {
		return pets_grooming_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'pets_grooming_merge_js' ) ) {
	/**
	 ...
	 */
	function pets_grooming_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= pets_grooming_fgc( pets_grooming_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = pets_grooming_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = pets_grooming_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			pets_grooming_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'pets-grooming' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'pets_grooming_filter_js_output', apply_filters( 'pets_grooming_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'pets_grooming_merge_css' ) ) {
	/**
	 ...
	 */
	function pets_grooming_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'pets_grooming_filter_responsive_sizes', pets_grooming_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = pets_grooming_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = pets_grooming_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'pets_grooming_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'pets_grooming_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'pets-grooming' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'pets-grooming' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = pets_grooming_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			pets_grooming_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'pets-grooming') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'pets_grooming_filter_css_output', apply_filters( 'pets_grooming_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'pets_grooming_filter_merge_css' ) ) {
	add_filter( 'pets_grooming_filter_merge_css', 'pets_grooming_filter_merge_css' );
	/**
	 * Replace all relative paths in the merged CSS-file to resolve url() attributes.
	 *
	 * @hooked 'pets_grooming_filter_merge_css'
	 *
	 * @param string $css  A merged CSS content to resolve paths.
	 *
	 * @return string      A processed CSS content
	 */
	function pets_grooming_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'pets_grooming_merge_sass' ) ) {
	/**
	 ...
	 */
	function pets_grooming_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'pets_grooming_filter_responsive_sizes', pets_grooming_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'pets_grooming_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			pets_grooming_fpc( pets_grooming_get_file_dir( $to ), apply_filters( 'pets_grooming_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'pets_grooming_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'pets_grooming_add_protocol' ) ) {
	/**
	 ...
	 */
	function pets_grooming_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? pets_grooming_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'pets_grooming_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function pets_grooming_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'pets_grooming_add_to_url' ) ) {
	/**
	 ...
	 */
	function pets_grooming_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = pets_grooming_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'pets_grooming_is_url' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'pets_grooming_is_external_url' ) ) {
	/**
	 ...
	 */
	function pets_grooming_is_external_url( $url ) {
		...
		return pets_grooming_is_url( $url ) && strpos( $url, pets_grooming_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}

if ( ! function_exists( 'pets_grooming_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function pets_grooming_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => pets_grooming_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => PETS_GROOMING_CHILD_DIR,
				'url' => pets_grooming_remove_protocol_from_url( PETS_GROOMING_CHILD_URL )
				),
			'theme' => array(
				'dir' => PETS_GROOMING_THEME_DIR,
				'url' => pets_grooming_remove_protocol_from_url( PETS_GROOMING_THEME_URL )
				)
			);
