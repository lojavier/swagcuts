 ...
 * @since PETS GROOMING 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'PETS_GROOMING_THEME_FREE' ) ) {
	define( 'PETS_GROOMING_THEME_FREE', false );
}
if ( ! defined( 'PETS_GROOMING_THEME_FREE_WP' ) ) {
	define( 'PETS_GROOMING_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'PETS_GROOMING_THEME_ALLOW_UPDATE' ) ) {
	define( 'PETS_GROOMING_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'PETS_GROOMING_ALLOW_SKINS' ) ) {
	define( 'PETS_GROOMING_ALLOW_SKINS', true );
}
if ( ! defined( 'PETS_GROOMING_DEFAULT_SKIN' ) ) {
	define( 'PETS_GROOMING_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'PETS_GROOMING_REMEMBER_SKIN' ) ) {
	define( 'PETS_GROOMING_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['PETS_GROOMING_STORAGE'] = array(
...
if ( ! function_exists( 'pets_grooming_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function pets_grooming_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = pets_grooming_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'pets_grooming_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'pets_grooming_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function pets_grooming_trx_updater_get_theme_info( $info ) {
		$engine = pets_grooming_storage_get( 'engine' );
...
if ( ! function_exists( 'pets_grooming_original_theme_slug' ) ) {
	add_filter( 'pets_grooming_filter_original_theme_slug', 'pets_grooming_original_theme_slug' );
	/**
	 ...
	 *
	 * @hooked 'pets_grooming_filter_original_theme_slug'
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function pets_grooming_original_theme_slug( $theme_slug = '' ) {
...
if ( ! function_exists( 'pets_grooming_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'pets_grooming_importer_set_options', 9 );
	/**
	 ...
	 */
	function pets_grooming_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( pets_grooming_get_protocol() . ':' . pets_grooming_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( pets_grooming_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'pets_grooming_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'pets_grooming_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function pets_grooming_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( pets_grooming_get_protocol() . ':' . pets_grooming_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'pets_grooming_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'pets_grooming_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function pets_grooming_customizer_theme_setup1() {
		pets_grooming_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'pets_grooming_filter_allow_fullscreen' return false)
