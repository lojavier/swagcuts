...
if ( ! function_exists( 'pets_grooming_customizer_get_css' ) ) {

	function pets_grooming_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = pets_grooming_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
CSS;
			$rez          = apply_filters( 'pets_grooming_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = pets_grooming_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'pets_grooming_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( pets_grooming_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = pets_grooming_get_scheme_colors( $s );
					}

					// Make theme-specific colors and tints
					$colors         = pets_grooming_customizer_add_theme_colors( $colors );
					$rez['colors']  = '';
					$rez            = apply_filters( 'pets_grooming_filter_get_css', $rez, array( 'colors' => $colors, 'scheme' => $s ) );
		...
		return apply_filters( 'pets_grooming_filter_prepare_css', $css_str, $remove_spaces );
	}
}
