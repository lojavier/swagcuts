...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			pets_grooming_create_observer( 'pets-grooming-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				pets_grooming_gutenberg_editor_init();
			} );
		} else {
		 	pets_grooming_gutenberg_editor_init();
		}
	...
	function pets_grooming_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'pets_grooming_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'pets_grooming_inited' ) ) {
	...
	// Init on page load
	function pets_grooming_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = pets_grooming_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + PETS_GROOMING_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + PETS_GROOMING_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in PETS_GROOMING_STORAGE['override_classes'] ) {
				$obj.addClass( PETS_GROOMING_STORAGE['override_classes'][i].replace( '%s', PETS_GROOMING_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( pets_grooming_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && PETS_GROOMING_STORAGE['sidebar_position'] != 'hide' ) {
				if ( pets_grooming_apply_filters( 'pets_grooming_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'pets_grooming_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( pets_grooming_apply_filters( 'pets_grooming_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				pets_grooming_remove_observer( 'pets-grooming-check-editor-styles-wrapper' );
				pets_grooming_create_observer( 'pets-grooming-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'pets_grooming_inited' );
	}
} );
